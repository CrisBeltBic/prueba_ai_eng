...
async def run_scraper(job_id: str, max_pages: int, delay: float) -> None:
    ...
    urls = await _crawl(max_pages, delay, status)

    status["phase"] = "scraping"
    await _scrape_and_save(urls, delay, status)

    status["phase"] = "done"
    status["finished_at"] = datetime.utcnow().isoformat()


async def _crawl(max_pages: int, delay: float, status: dict) -> list[str]:
    visited: set[str] = set()
...
async def _scrape_and_save(urls: list[str], delay: float, status: dict) -> None:
